...
#define FOREGROUND_STAR_IMAGE_NAME @"xing"
#define BACKGROUND_STAR_IMAGE_NAME @"xing_kong"
...
- (void)setwoScorePercent:(CGFloat)woscroePercent {
//    if (!_allowEditStar) {
//        return;
//    }
    if (_scorePercent == woscroePercent) {
        return;
    }
    
    if (woscroePercent < 0) {
        _scorePercent = 0;
    } else if (woscroePercent > 1) {
        _scorePercent = 1;
    } else {
        _scorePercent = woscroePercent;
    }
    
    if ([self.delegate respondsToSelector:@selector(starRateView:scroePercentDidChange:)]) {
        [self.delegate starRateView:self scroePercentDidChange:woscroePercent];
    }
